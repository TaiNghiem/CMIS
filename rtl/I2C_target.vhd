...
architecture rtl of I2C_target is
    ...
    type t_target_state is (idle, get_first_byte,
                        send_first_ACK, send_ACK,
                        receive_ACK, send_byte,
                        receive_byte, wait_op,
                        send_offset_ACK, receive_offset);
        ...
        if rising_edge(clk) then
            if rst = '1' then
                ...
                case state is
                    ...
                    when send_first_ACK =>
                        ...
                        if SCL_falling_edge = '1' then
                            ...
                            if write_trans = '1' then
                                state <= receive_offset;
                            elsif read_trans = '1' then
                                state <= wait_op;
                            end if;
                        end if;
                    ...
                    when send_offset_ACK =>
                        if offset_NACK = '1' then
                            SDA_out <= 'Z';
                            state <= idle;
                        else
                            SDA_out <= '0';
                            if SCL_rising_edge = '1' then
                            current_addr <= byte_reg;
                            elsif SCL_falling_edge = '1' then
                                SDA_out <= 'Z';
                                state <= receive_byte;
                            end if;
